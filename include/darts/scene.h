...
public:
    ...
    Box3f bounds() const override { return m_surfaces->bounds(); }

    /// Return the background color
    Color3f background(const Ray3f &ray) const;

    /// Return the camera
    shared_ptr<const Camera> camera() const { return m_camera; }
    ...
    */
    ...
    Image3f raytrace(const string &outfile, const string &outfile_hdr) const;

private:
    json                     m_json; ///< The original json data used to create this scene
