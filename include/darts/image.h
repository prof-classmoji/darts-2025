...
public:
     ...
     */
    Image(int w, int h, const T &v) : Base(w, h) { reset(v); }
    ...
    */
    ...
    /**
        ...
        \param metadata Optional metadata to include in the saved file (only supported in EXR files)
        \return 		True if the file saved successfully
     */
    bool save(const std::string &filename, float gain = 1.0f, const nlohmann::json &metadata = nlohmann::json{});

    /// Set of supported formats for image loading
    static std::set<std::string> loadable_formats()
    {
        return {"jpg", "jpeg", "png", "bmp", "psd", "tga", "gif", "hdr", "pic", "ppm", "pgm", "exr"};
    }

    /// Set of supported formats for image saving
    static std::set<std::string> savable_formats() { return {"bmp", "exr", "hdr", "jpg", "png", "tga"}; }
};
...
template <>
bool Image4f::load(const string &filename, bool raw);

template <>
bool Image3f::save(const string &filename, float gain, const nlohmann::json &metadata);
template <>
bool Image4f::save(const string &filename, float gain, const nlohmann::json &metadata);
