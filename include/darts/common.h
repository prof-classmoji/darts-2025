...
// using std::clamp; // this conflicts with la::clamp, which works properly for n-D vectors too
using std::make_pair;
...
bit_cast(const From &src) noexcept
{
    static_assert(std::is_trivially_constructible_v<To>,
                  "This implementation of bit_cast requires destination type to be trivially constructible");
...
public:
    ...
    public:
        ...
        const Vec<N, T> &operator*() const { return a; }
        Iterator        &operator++()
        {
            for (int d = 0; d < N; ++d)
            {
        ...
        bool operator!=(const Iterator &other) { return a[N - 1] != other.a[N - 1]; }
    };

    RangeND(const Vec<N, T> &a, const Vec<N, T> &b) : m_start(a), m_end(b) {}

    Iterator begin() const { return Iterator(*this, m_start); }
    Iterator end() const { return Iterator(*this, m_end); }
...
inline RangeND<2, int> range2i(const Vec<2, int> &a, const Vec<2, int> &b) { return range(a, b); }

inline RangeND<3, int> range3i(const Vec<3, int> &a, const Vec<3, int> &b) { return range(a, b); }

inline RangeND<4, int> range4i(const Vec<4, int> &a, const Vec<4, int> &b) { return range(a, b); }
...
inline RangeND<2, int> range2i(const Vec<2, int> &b) { return range(Vec<2, int>{0}, b); }

inline RangeND<3, int> range3i(const Vec<3, int> &b) { return range(Vec<3, int>{0}, b); }

inline RangeND<4, int> range4i(const Vec<4, int> &b) { return range(Vec<4, int>{0}, b); }
