...
{
    return std::acos(clamp(v.z, T(-1), T(1)));
}
...
T sin_phi(const Vec3<T> &v)
{
    ...
    if (sin_theta == T(0))
        return T(1);
    return clamp(v.y / sin_theta, T(-1), T(1));
}
...
T cos_phi(const Vec3<T> &v)
{
    ...
    if (sin_theta == T(0))
        return T(1);
    return clamp(v.x / sin_theta, T(-1), T(1));
}
...
{
    return clamp(v.y * v.y / sin_theta2(v), T(0), T(1));
}
...
{
    return clamp(v.x * v.x / sin_theta2(v), T(0), T(1));
}
...
/**
    ...
    \param [in] phi_theta
        ...
        \f$\theta = 0 \mapsto (0,0,1)\f$ and \f$\theta = \pi \mapsto (0,0,-1)\f$.
    \return
...
/**
    Convert cylindrical (phi,z) coordinates to a unit direction in Cartesian coordinates

    \param [in] phi_z
        The azimuthal angle \f$\phi \in [0,2\pi)\f$ and z-coordinate.
    \return
        The corresponding unit-length direction vector in Cartesian coordinates.
*/
template <typename T>
Vec3<T> cylindrical_coordinates_to_direction(const Vec2<T> &phi_z)
{
    auto [sin_phi, cos_phi] = sincos(phi_z.x);

    return Vec3<T>{cos_phi, sin_phi, phi_z.y};
}

/**
    Convert a direction from Cartesian coordinates to cylindrical (phi,z) coordinates

    \param [in] v
        The direction vector in Cartesian coordinates (not necesarily unit length)
    \return
        The azimuthal angle \f$\phi \in [0,2\pi)\f$.
*/
template <typename T>
Vec2<T> direction_to_cylindrical_coordinates(const Vec3<T> &v)
{
    return {phi(v), v.z};
}
