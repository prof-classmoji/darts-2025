...
public:
    ...
    RNG(uint64_t sequence_index, uint64_t init_state) { seed(sequence_index, init_state); }
    RNG(uint64_t sequence_index) { seed(sequence_index); }
    ...
    void seed(uint64_t sequence_index) { seed(sequence_index, mix_bits(sequence_index)); }
    ...
    uint32_t rand1u(uint32_t range)
    {
        ...
        if (l < range)
        {
            uint32_t t = int32_t(0) - range; // -range mod 2^32
            if (t >= range)
            {
                t -= range;
                if (t >= range)
    ...
    float rand1f() { return bit_cast<float>((rand1u() >> 9) | 0x3f800000u) - 1.0f; }
    ...
    */
    ...
    {
        for (Iterator it = end - 1; it > begin; --it) std::iter_swap(it, begin + rand1u((uint32_t)(it - begin + 1)));
    }
    ...
    */
    ...
    /// Equality operator
    bool operator==(const RNG &other) const { return state == other.state && inc == other.inc; }
