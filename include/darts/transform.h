...
struct Transform
{
    ...
    Transform inverse() const { return Transform(m_inv, m); }

    /// Concatenate with another transform
    Transform operator*(const Transform &t) const { return Transform(mul(m, t.m), mul(t.m_inv, m_inv)); }
    ...
    Box3f box(const Box3f &box) const
    {
        ...
        for (size_t i = 1; i < 8; ++i) new_box.enclose(point(pts[i]));
