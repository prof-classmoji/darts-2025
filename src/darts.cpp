...
The default is 2 (info).)")
    ...
    try
    {
        ...
        {
            spdlog::info("Manually setting number of threads in thread pool to {}.",
                         ThreadPool::singleton(threads)->size());
        }
        else
        {
            spdlog::info("Automatically setting number of threads in thread pool to {}.",
                         ThreadPool::singleton()->size());
        }
        ...
        auto image = scene->raytrace(outfile, outfile_hdr);
    }
