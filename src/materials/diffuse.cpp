...
/// A perfectly diffuse material. \ingroup Materials
class Diffuse : public Material
{
public:
    Diffuse(const json &j = json::object());

    bool scatter(const Ray3f &ray, const HitRecord &hit, Color3f &attenuation, Ray3f &scattered) const override;


    Color3f albedo = Color3f(0.8f); ///< The diffuse color (fraction of light that is reflected per color channel).
};

Diffuse::Diffuse(const json &j) : Material(j)
{
    albedo = j.value("color", albedo);
}

bool Diffuse::scatter(const Ray3f &ray, const HitRecord &hit, Color3f &attenuation, Ray3f &scattered) const
{
