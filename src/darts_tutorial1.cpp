...
void test_camera_class_image()
{
    ...
    Camera camera({{"vertical fov", 90.f},
...
void test_ray_sphere_intersection()
{
    ...
    auto   material = DartsFactory<Material>::create(json{{"type", "diffuse"}, {"color", 1.f}});
...
void test_materials()
{
    ...
    // TODO: Look at material.h|cpp and then go implement the Diffuse and Metal materials in diffuse.cpp and
    // metal.cpp, respectively

    // Note the line at the end of both files that looks something like this:
    // DARTS_REGISTER_CLASS_IN_FACTORY(Material, Diffuse, "diffuse")
    // This macro creates some code that informs a "Factory" how to create Materials of type Diffuse, and allows us
    // to later create them by providing the key "diffuse".

    // Let's see how this works by creating a red Diffuse material.
    //
    ...
    // this case it is "diffuse", which matches the third parameter in the DARTS_REGISTER_CLASS_IN_FACTORY macro
    // above
    Color3f surface_color    = Color3f(1.0f, 0.25f, 0.25f);
    auto    lambert_material = DartsFactory<Material>::create(json{{"type", "diffuse"}, {"color", surface_color}});

    // And now let's create a slightly shiny metal surface
    auto metal_material =
        DartsFactory<Material>::create(json{{"type", "metal"}, {"color", surface_color}, {"roughness", 0.3f}});
    ...
    spdlog::info("Testing Diffuse::scatter");
    ...
    if (max_lambert_error > 1e-5f)
        spdlog::error("Diffuse scattered origin and attenuation error too high. Result might be incorrect!");

    if (scattering_failed)
        spdlog::error("Diffuse incorrect! Scatter function returned false\n");
...
void test_recursive_raytracing()
{
    ...
    auto ground = DartsFactory<Material>::create(json{{"type", "diffuse"}, {"color", 0.5f}});
    auto matte  = DartsFactory<Material>::create(json{{"type", "diffuse"}, {"color", Vec3f(1.0f, 0.25f, 0.25f)}});
    auto shiny  = DartsFactory<Material>::create(json{{"type", "metal"}, {"color", 1.f}, {"roughness", 0.3f}});
...
Color3f vec2color(const Vec3f &dir) { return 0.5f * (dir + 1.f); }

Color3f ray2color(const Ray3f &r) { return vec2color(normalize(r.d)); }
