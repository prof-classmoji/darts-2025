...
bool load(const string &filename, bool raw, Image<Color<N, float>> &image)
{
    ...
    // next try exrs
    {
        ...
        if (ret == TINYEXR_SUCCESS)
        {
            ...
            // copy pixels over to the Image
            for (auto y : range(h))
                for (auto x : range(w)) image(x, y) = Color<N, float>{float_data + 4 * (x + y * w)};
...
template <int N>
bool save(const string &filename, float gain, const Image<Color<N, float>> &buffer, const json &metadata)
{
    ...
    else if (extension == "exr")
    {
        ...
        for (auto i : range(N))
        {
            ...
            for (auto j : range(images[i].size())) images[i][j] = buffer(j)[i];
        }

        float *image_ptr[N];
        // first 3 channels are in BGR order, then A
        for (auto i : range(3)) image_ptr[i] = &(images[2 - i].at(0));
        if constexpr (N == 4)
        ...
        for (auto i : range(N)) strncpy(header.channels[i].name, chan_names[i], 255);
        ...
        header.custom_attributes = new EXRAttribute[metadata.size()];
        std::vector<uint8_t *> allocated_values; // Track allocated pointers for cleanup
        int                    attr_index = 0;
        for (auto it = metadata.begin(); it != metadata.end(); ++it)
        {
            if (it.value().is_string())
            {
                strncpy(header.custom_attributes[attr_index].name, it.key().c_str(), 255);
                strncpy(header.custom_attributes[attr_index].type, "string", 255);
                string   val      = it.value().get<string>();
                size_t   val_size = val.size() + 1; // Include null terminator
                uint8_t *val_ptr  = new uint8_t[val_size];
                memcpy(val_ptr, val.c_str(), val_size);
                header.custom_attributes[attr_index].value = val_ptr;
                header.custom_attributes[attr_index].size  = val_size;
                allocated_values.push_back(val_ptr);
                ++attr_index;
            }
        }
        header.num_custom_attributes = attr_index;
        ...
        delete[] header.custom_attributes;
        for (auto ptr : allocated_values) delete[] ptr;
...
template <>
bool Image<Color3f>::save(const string &filename, float gain, const json &metadata)
{
    return ::save(filename, gain, *this, metadata);
}
...
template <>
bool Image<Color4f>::save(const string &filename, float gain, const json &metadata)
{
    return ::save(filename, gain, *this, metadata);
}
