...
{
public:
    bool intersect(const Ray3f &ray_, HitRecord &hit) const override
    {
    ...
    {
        Box3f b;
        for (auto &s : surfaces) b.enclose(s->bounds());
...
struct BBHNode : public Surface
{
    ...
    Box3f bounds() const override { return bbox; }
};
...
bool BBHNode::intersect(const Ray3f &ray_, HitRecord &hit) const
{
    ++bbh_nodes_visited;
...
BBH::BBH(const json &j) : SurfaceGroup(j)
{
    ...
    max_leaf_size = j.value("max leaf size", max_leaf_size);

    string sm = j.value("split method", "equal");
    if (sm == "sah")
...
void BBH::build(const Scene *scene)
{
...
bool BBH::intersect(const Ray3f &ray_, HitRecord &hit) const
{
    ++total_rays;
    if (!root)
        return false;
