    ...
    m_update_thread = std::thread(
        [this]()
        {
            ...
            do {
                ...
                double fraction = saturate(double(m_steps_done) / double(m_num_steps));
                ...
                if (!done)
                {
                    ...
                    std::this_thread::sleep_for(clamp(std::chrono::milliseconds(int(elp * 0.01)), 40ms, 10000ms));
                }
            } while (!done);
        });
}

Progress::~Progress() { set_done(); }
