...
{
    for (AccumFunc func : *stat_funcs) func(accum);
}
...
void StatsAccumulator::report_counter(const char *name, int64_t val) { stats->counters[name] += val; }

StatsAccumulator::StatsAccumulator() { stats = new Stats; }

void StatsAccumulator::report_memory_counter(const char *name, int64_t val) { stats->memory_counters[name] += val; }
...
std::string stats_report() { return stats_accumulator.report(); }

void clear_stats() { stats_accumulator.clear(); }
...
std::string StatsAccumulator::report()
{
    ...
    for (auto &categories : to_print)
    {
        ...
        for (auto &item : categories.second) dest += fmt::format("    {}\n", item);
    }
